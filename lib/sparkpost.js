...
const appendQuery = function (uri, qs) {
  if (!qs || typeof qs !== 'object' || Object.keys(qs).length === 0) {
    return uri
  }

  const parsed = url.parse(uri, true)
  parsed.query = Object.assign({}, parsed.query, qs)
...
const buildFetchOptions = function (options, defaultHeaders) {
  ...
  const fetchOptions = {
    method,
    headers,
    // node-fetch decompresses gzip by default; honor the old request `gzip` flag
...
const attachDebug = function (payload, debug, fetchOptions, uri, res) {
  if (!debug || payload === null || typeof payload !== 'object') {
    return payload
  }
...
const handleResponse = function (res, debug, fetchOptions, uri) {
  ...
  return res.text().then(function (text) {
    ...
    if (invalidCodeRegex.test(res.status)) {
      throw createSparkPostError({ statusCode: res.status, statusMessage: res.statusText }, typeof body === 'object' ? body : {})
    }

    return attachDebug(body, debug, fetchOptions, uri, res)
  })
}
...
SparkPost.prototype.request = function (options, callback) {
  ...
  options.uri = appendQuery(resolveUri(baseUrl, options.uri), options.qs)
